...
class Stat:
    ...
    def AR(self, p: int):
        try:
            ...
            forecasts = self.showForecast(forecast_values, "AR("+str(p)+")")
            return ([model_fit.summary(), forecasts])
    ...
    def MA(self, q: int):
        try:
            ...
            model_fit = model.fit()
            forecast_values = model_fit.forecast(10)
            forecasts = self.showForecast(forecast_values, "MA("+str(q)+")")
            return ([model_fit.summary(), forecasts])
    ...
    def ARMA(self, p:int, q: int):
        try:
            ...
            model_fit = model.fit()
            forecast_values = model_fit.forecast(10)
            forecasts = self.showForecast(forecast_values, "ARMA("+str(p)+", "+str(q)+")")
            return ([model_fit.summary(), forecasts])
    ...
    def ARIMA(self, p:int, d: int, q: int):
        try:
            ...
            model_fit = model.fit()
            forecast_values = model_fit.forecast(10)
            forecasts =  self.showForecast(forecast_values, "ARIMA("+str(p)+", "+str(d)+", "+(q)+")")
            return ([model_fit.summary(), forecasts])
